...
def cov_matrix(x):
    ...
    # RXX1 = x @ np.conjugate(np.roll(x.T,2,axis=0))
    # RXX2 = np.conjugate(np.roll(x.T,2,axis=0)).T @ x.T
    # Rxx = RXX1 * RXX2 * 0.25
...
def cov_matrix_x1_x2(x1,x2):
    ...
    Rxx = np.divide(Rxx*Rxx1/4, num_adc_samples)
...
def gen_steering_vec_el(ang_est_range, ang_est_resolution, num_ant):
    """Generate a steering vector for AOA estimation given the theta range, theta resolution, and number of antennas
...
def gen_steering_vec_2D(ang_est_range, ang_est_resolution, num_ant):
    kelm = 4
    kelm2 = 4
    dd = 0.5
    d = np.arange(0, kelm*dd, dd)
    d2 = np.arange(0, kelm2*dd, dd)
    twpi = 2*np.pi
    derad = np.pi/180
    num_vec = (2 * ang_est_range / ang_est_resolution + 1)
    num_vec = int(round(num_vec))
    steering_vectors = np.zeros((num_vec*num_vec, 12), dtype='complex64')
    for kk in range(num_vec):
        for jj in range(num_vec):
            # a1=np.exp(-1j*twpi*d*np.sin(derad*kk)*np.sin(derad*jj)).T
            # a2=np.exp(-1j*twpi*d2*np.sin(derad*kk)*np.cos(derad*jj)).T
            a1 = np.exp(-1j * np.pi * d * np.sin((-ang_est_range + kk * ang_est_resolution) * np.pi / 180) * np.sin((-ang_est_range + jj * ang_est_resolution) * np.pi / 180)).T
            a2 = np.exp(-1j * np.pi * d2 * np.sin((-ang_est_range + kk * ang_est_resolution) * np.pi / 180) * np.cos((-ang_est_range + jj * ang_est_resolution) * np.pi / 180)).T
            a = np.kron(a2,a1)
            a = np.delete(a,(8,9,12,13),0)
            steering_vectors[kk*num_vec+jj,:] = a
...
def naive_xyz(virtual_ant, num_tx=3, num_rx=4, fft_size=64):
    ...
    return x_vector, y_vector, z_vector, azimuth_fft
